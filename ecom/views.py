...
def home_view(request):
    ...
    # Calculate cart count from CartItem model
    cart_count = models.CartItem.objects.filter(session_key=session_key).count()
    
    if request.user.is_authenticated:
        return HttpResponseRedirect('afterlogin')
    return render(request,'ecom/index.html',{'products':products,'product_count_in_cart':cart_count})
...
def search_view(request):
    ...
    if request.user.is_authenticated:
        return render(request,'ecom/customer_home.html',{'products':products,'word':word,'product_count_in_cart':cart_count})
    return render(request,'ecom/index.html',{'products':products,'word':word,'product_count_in_cart':cart_count})
...
def add_to_cart_view(request,pk):
    ...
    response = render(request, 'ecom/index.html', {
        'products': products,
        'product_count_in_cart': cart_count
    })
...
def remove_from_cart_view(request,pk):
    ...
    try:
        cart_item = models.CartItem.objects.get(product_id=pk, session_key=session_key)
        cart_item.delete()
